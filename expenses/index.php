...
$result = $conn->query('SELECT id, date, category, amount FROM expenses');
...
<body>
    ...
    <div class="container mt-5">
        <div class="d-flex justify-content-between">
            <h1>Expenses List</h1>
            <div>
                <a href="create.php" class="btn btn-info">Create Expense</a>
            </div>
        </div>
                    ...
                    if ($result->num_rows > 0) {
                        ...
                        while ($row = $result->fetch_assoc()) {
                            ...
                            echo "<tr>
                                    <td>{$sNo}</td>
                                    <td>{$row['date']}</td>
                                    <td>" . EXPENSE_CATEGORIES[$row['category']] . "</td>
                                    <td>{$row['amount']}</td>
                                    <td>
